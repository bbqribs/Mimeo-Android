...
- **Merge authority**: Claude and Codex may merge pull requests only when explicitly instructed by the operator. Without explicit merge instruction, stop after opening or updating the PR and report `not merged; awaiting operator approval`. Owning the ticket or branch is not a merge entitlement. See `AGENTS.md §Merge authority (canonical)`.
...
- **Shared lifecycle hygiene**: See `AGENTS.md §Ticket lifecycle hygiene` for preflight, implementation discipline, PR/open report, post-merge closeout, and merge trigger rules that apply to all agents including Codex.
