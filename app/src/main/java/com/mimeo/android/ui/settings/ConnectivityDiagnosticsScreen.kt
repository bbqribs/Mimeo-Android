...
fun ConnectivityDiagnosticsScreen(vm: AppViewModel) {
    ...
    Column(
        ...
        if (isPhysicalDevice) {
            Text("Physical phone quick check: LAN mode -> http://<PC_LAN_IP>:8000 (or HTTPS if LAN TLS is configured), Remote mode -> HTTPS-first ($DEFAULT_REMOTE_BASE_URL). Enter fallback HTTP manually only when TLS is disabled. 10.0.2.2 is emulator-only.")
        } else {
