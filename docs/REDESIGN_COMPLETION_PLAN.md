...
| **Smart playlist** | Sectioned Smart Playlist. Filter summary header. Pinned section + live-matching section. Sort controls. Pin/unpin per row. Freeze as manual playlist as secondary. Smart playlist may seed Up Next via explicit action; never auto-mutates Up Next. | Aligns with product model Model B (saved filter + optional manual pin/order layer). Avoids smart playlist behaving as live Up Next. | Android read/display is shipped with row tap-to-Locus, smart row Play Now / Play Next / Play Last, batch Add Selected to Up Next, explicit "Use as Up Next" seeding, pin/unpin/reorder, and Freeze-as-manual. Seeding uses displayed rows in displayed order as a snapshot, confirms replacement of non-empty Up Next, labels the source `Smart view: {name}`, and adds no backend/API change, live sync, cross-device continuity, or manual playlist reseed change. Remaining gaps: Android create/edit/delete only. | Treating a smart playlist as Up Next continuity. Unclear archive/binned policy for pinned items. |
...
8. **C2 — Smart playlist pinned/live surface.**
   Android read/display plus explicit Up Next seed, pin/unpin/reorder,
   and Freeze-as-manual are shipped. Remaining Android work is
   create/edit/delete UI. Pin behavior under archive/bin/filter change
   remains a product-policy follow-up.
