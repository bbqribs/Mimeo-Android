...
- Treat hardware `KEYCODE_MEDIA_PAUSE` as state-aware while Mimeo owns the active item: pause if Mimeo is currently playing, but resume if Mimeo is already paused. Some headset/router combinations keep emitting `PAUSE` for the next physical toggle after Mimeo pauses, even though the user expects play/pause toggle behavior.
